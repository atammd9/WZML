...
handler_dict = {}
example_dict = {'prefix':'1. <code>@username or Anything</code>', 
                'mprefix':'1. <code>@username or Anything</code>', 
                'suffix':'1. <code>~ Username</code>\n2. <code>~ @username</code>', 
                'msuffix':'1. <code>~ Username</code>\n2. <code>~ @username</code>', 
...
def get_user_settings(from_user, key=None):
    ...
    if key is None:
        ...
        text = f'''<u>Universal Settings for</u> <a href='tg://user?id={user_id}'>{name}</a>
...
'''
        ...
        buttons.sbutton("Close", f"userset {user_id} close", 'footer')
        button = buttons.build_menu(2)
        text = f'''<u>Mirror/Clone Settings for</u> <a href='tg://user?id={user_id}'>{name}</a>
...
'''
        ...
        buttons.sbutton("Close", f"userset {user_id} close", 'footer')
        button = buttons.build_menu(2)
        text = f'''<u>Leech Settings for</u> <a href='tg://user?id={user_id}'>{name}</a>
